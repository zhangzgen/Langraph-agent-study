...
- **持久会话与上下文管理**：使用 SQLite 或 PostgreSQL checkpoint 保存多轮状态，支持中断后恢复执行；长上下文可自动压缩为摘要，过期工具结果也可按 KV cache TTL 替换为占位符。
...
flowchart LR
    ...
    subgraph plan["可选 Plan 子图"]
        ...
        plan_llm -->|"tool_calls"| plan_tools --> expire
        plan_llm -->|"无工具调用"| review_plan
        review_plan -->|"需调整 / 审核通过"| expire
    end

    subgraph react["主 ReAct 执行图"]
        expire["expire_tool_results\n过期工具结果占位压缩"]
    ...
    agent_graph -->|"--plan"| expire
    agent_graph -->|"普通模式"| expire
    review_plan -->|"审核通过"| expire
...
- `START -> expire_tool_results -> llm/plan_llm`：每次模型 API 调用前先检查工具结果是否已超过 KV cache TTL，必要时替换为占位符。
- `expire_tool_results -> plan_llm -> review_plan -> expire_tool_results -> llm`：只在 CLI 启用 `--plan` 时出现，计划书通过后进入主执行图。
- `llm -> classify_tool_calls -> approval_gate -> execute_tools -> expire_tool_results -> llm`：主 ReAct 工具循环，高风险工具通过 `interrupt()` 暂停等待人工确认。
...
├── context.py             # token 统计、历史摘要、工具结果占位与上下文物理裁剪
...
LANGRAPH_TOOL_RESULT_COMPACTION_ENABLED=true
LANGRAPH_KV_CACHE_TTL_SECONDS=300
...
- SQLite/PostgreSQL checkpoint 选择、连接串隐藏、上下文摘要压缩及过期工具结果占位压缩。
...
1. 每次进入 `llm` 或 `plan_llm` 前，`expire_tool_results` 会先检查 `LANGRAPH_TOOL_RESULT_COMPACTION_ENABLED`，开启时再检查 `last_model_or_turn_at`、上一条 assistant 是否存在以及 KV cache TTL，过期时把全量 `messages` 中的所有 `ToolMessage` 正文替换为占位符。
2. 启用 plan 模式时，图先让计划模型使用 `ask_human` 或只读文件工具补充上下文，并等待用户审核计划书；否则直接进入主执行模型。
3. 主执行模型推理并决定是否需要工具。
4. 如果返回 `tool_calls`，条件边把状态路由到 `classify_tool_calls`。
5. `classify_tool_calls` 把调用分为自动批准和待人工审核，并写入 `pending_approvals`、`approved_tool_calls` 和 `tool_audit_log`。
6. 如果存在 `pending_approvals`，`approval_gate` 通过 `interrupt()` 暂停并等待用户逐项批准或拒绝。
7. `execute_tools` 执行已批准的工具，并为被拒绝的工具调用生成 `ToolMessage`。
8. 图回到 `expire_tool_results -> llm`，模型读取未过期工具结果或占位符后继续推理。
9. 当最终回答达到压缩阈值且历史足够长时，图进入 `summarize_and_compact`；否则路由到 `END`。
...
另外，图在每次调用模型 API 前都会先经过 `expire_tool_results` 节点。`LANGRAPH_TOOL_RESULT_COMPACTION_ENABLED=true` 时，该节点读取 `last_model_or_turn_at`，当距离上一次模型输出已经超过 `LANGRAPH_KV_CACHE_TTL_SECONDS`，且历史里已有 assistant 消息时，会用占位符替换全量 `messages` 中所有 `ToolMessage` 的正文，同时保留 `tool_call_id`、工具名和消息顺序，避免 KV cache 过期后继续携带大体积工具输出。`llm`、`plan_llm`、普通模式、plan 模式和工具执行后的回环都会使用同一个节点。

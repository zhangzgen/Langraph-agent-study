...
└── tools/
    ...
    ├── filesystem.py      # LangChain FileManagementToolkit 文件工具
...
需要查看或修改项目文件时，Agent 会使用 LangChain 的 `FileManagementToolkit` 文件工具，范围限制在当前项目根目录：

- 自动执行：`read_file`、`list_directory`、`file_search` 等只读工具。
